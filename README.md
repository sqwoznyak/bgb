...
1. **Client-table**:
   - Хранит информацию о пользователях:
     ...
     - `role` (tesxt): роль пользователя (например, пользователь, администратор или стартовый).
     - `referal_id` (integer): ID реферера для отслеживания реферальных связей. Пока не используется.
     - `created_time` (integer): дата регистрации.
     - `active` (boolean): флаг активности чата пользователя.Пока не используется.

2. **Transaction_table**:
   - Хранит данные о транзакциях, связанных с пользователями:
     - `tg_id` (integer): идентификатор пользователя, связанный с транзакцией.
     - `description` (text): текстовое описание срока продления.
     - `json_config` (text): Поле с id платежа.

3. **key_table**:
   - Хранит данные о ключах доступа:
     - `tg_id` (integer): идентификатор пользователя, связанный с ключом.
     - `key_name` (text): название ключа.
     - `start_date` (integer): дата старта действия ключа.
     - `end_date` (integer): дата истечения срока действия ключа.
...
4. **Servers_table**:
   - Хранит информацию о VPN-серверах:
     ...
     - `server_status` (boolean): статус сервера (активен/неактивен).
     - `server_ip` (text): ip адресс сервера

#### **Связи между таблицами**
- **Client-table**:
  - Связана с `Transaction_table` и `key_table` через поле `tg_id`.
- **Transaction_table**:
